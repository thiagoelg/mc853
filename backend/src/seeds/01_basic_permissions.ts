          ...
          return knex("permission")
            ...
            .then((permissions: { id: number; short_name: string }[]) => {
              knex("role")
                ...
                .then(() => Promise.all([
                  knex("role")
                    .insert({ name: "Administrador", short_name: "admin", is_default: false }, "id")
                    .then(([role_id]) => {
                  ...
                  knex("role")
                    .insert({ name: "Gerente", short_name: "manager", is_default: false }, "id")
                    .then(([role_id]) => {
                  ...
                  knex("role")
                    .insert({ name: "Atendente", short_name: "worker", is_default: false }, "id")
                    .then(([role_id]) => {
                      const my_permissions = permissions.filter((perm) =>
                        [
                          Permission.shortNames.MANAGE_FORMS,
                  ...
                  knex("role")
                    .insert(
                      { name: "Solicitante", short_name: "requester", is_default: true },
                      "id"
                    )
                    .then(([role_id]) => {
                      const my_permissions = permissions.filter((perm) =>
